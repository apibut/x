...
//apikey
const apivhtear = 'beli' // Vhtear Beli Sendiri Yoo
const ZeksApi = 'apivinz'
const apileys = 'demo'
const TobzKey = 'BotWeA'
const lolhuman = '7f5a6556983b0bf183028c20' //Free Trial 3 day
const onlydev = 'beli' //Free Apikey
...
//owner number
const ownerNumber = ["6281325714976@s.whatsapp.net"]  //Ganti Jadi Nomormu
...
const akira = new WAConnection()
   ...
   console.log(color('[','aqua'),color('𝐓-𝐁𝐎𝐓 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋','white'),color(']','aqua'),color('SQAN QR CODE DI WHATSAPP WEB!!','aqua'),color('You','white'),color('Tube','red'),color('AkiRa','yellow'))
}) 
		...
		try {
			...
			if (anu.action == 'add') {
				num = anu.participants[0]
					pp_user = 'https://pm1.narvii.com/6482/f191b545a37f76db13b6bcde988820e5d3ae819c_hq.jpg'
				teks = `
ʜᴀʟʟᴏ
@${num.split('@')[0]}
...
//ANTI VIRTEX 
		...
		apiKey = args[0]
                    ...
                    case 'ytkomen':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nContoh: ${prefix + command} adul api.lolhuman.xyz`)
		username = args[0]
		comment = args[2]
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    case 'artinama':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} adul`)
                    ini_nama = args.join(" ")
...
case 'alay': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: Adul`)
...
// 
		...
		if (isBanned) return reply(ind.baned())
                ...
                case 'fakedonald':
                    ...
                    break
                case 'faketoko':
                    await faketoko(teks = "TOKO REBAHAN", url_image = "https://i.ibb.co/JdfQ73m/photo-2021-02-05-10-13-39.jpg", title = "𝐓-𝐁𝐎𝐓 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋", code = "IDR", price = 1000000)
                    break
                case 'ktpmaker':
                    if (args.length == 0) return reply(`Usage: ${prefix + command} nik|provinsi|kabupaten|nama|tempat, tanggal lahir|jenis kelamin|jalan|rt/rw|kelurahan|kecamatan|agama|status nikah|pekerjaan|warga negara|berlaku sampai|url_image\n\nContoh: ${prefix + command} 456127893132123|bumipertiwi|fatamorgana|Adul|mars, 99-99-9999|belum ditemukan|jl wardoyo|999/999|turese|imtuni|alhamdulillah islam|jomblo kack|mikirin dia|indo ori no kw|hari kiamat|https://i.ibb.co/Xb2pZ88/test.jpg`)
		...
		if (isBanned) return reply(ind.baned())
				...
				akira.sendMessage(from, ''+tes+'\n\n_By : 𝐓-𝐁𝐎𝐓 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋._', text, { quoted: mek })
				await limitAdd(sender)
				break
			case 'facebookpage':
	if (!isRegistered) return reply(ind.noregis())
...
//CASE BADWORD
		...
		if (isBanned) return reply(ind.baned())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Textnya Mana Cuy?\n*Contoh ${prefix}anakharamserti jstfreenet*`)
				reply(ind.wait())
				ct = body.slice(15)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Textnya Mana Cuy?\n*Contoh ${prefix}hekel jstfreenet*`)
				reply(ind.wait())
				ct = body.slice(11)
				...
				akira.sendMessage(from, anu, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "*Subscribe jstfnee*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, caption: 'Nih Hasilnya Kak...'})
				break
				case 'pubgtourserti': 				
				// Subscribe Yoo
				if (!isRegistered) return reply(ind.noregis())
...
break		     		
		...
		if (isBanned) return reply(ind.baned())
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Text Mana Cuy?\n*Contoh ${prefix}valorantbanner adul|Ganteng|Banget*`)
				ct = body.slice(15)
				ll1 = ct.split("|")[0];
                ll2 = ct.split("|")[1];
				ll3 = ct.split("|")[2];
				...
				if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Text Mana Cuy?\n*Contoh ${prefix}pemainbola adul|17*`)
				ct = body.slice(11)
				dap1 = ct.split("|")[0];
                dap2 = ct.split("|")[1];
				...
				if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Text Mana Cuy?\n*Contoh ${prefix}pemainbola adul|Ganz*`)
				ct = body.slice(11)
				dap1 = ct.split("|")[0];
                dap2 = ct.split("|")[1];
				...
				if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Text Mana Cuy?\n*Contoh ${prefix}galaxybat adulsu*`)
				ct = body.slice(10)
...
break
		...
		if (!isRegistered) return reply(ind.noregis())
                    if ((isMedia && !lol.message.videoMessage || isQuotedSticker)) {
                        if (args.length == 0) return reply(`Contoh: ${prefix + command} adul|Ganz`)
                        const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(lol).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        filePath = await akira.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
                        file_name = getRandom(".webp")
                        ini_txt = args.join(" ").split("|")
		...
		case 'daftar':
                ...
                pp_user = await getBuffer(`https://cdn-image.hipwee.com/wp-content/uploads/2018/01/hipwee-it-looks-like-animals-have-caught-the-selfie-virus-too-8-640x480.jpg`)
                veri = sender
                if (isGroup) {
...
𝐓-𝐁𝐎𝐓 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋
❏ *ᴡᴇʙ SSH:* 
http://jomblossh.studio/
❏ *ᴡᴇʙ V2RAY:* 
http://bucin.ga
❏ *ɪɴsᴛᴀɢʀᴀᴍ:* 
https://instagram.com/m.faisal_zx
❏ *𝙆𝙤𝙣𝙩𝙖𝙠 𝙤𝙬𝙣𝙚𝙧 𝙩-𝙗𝙤𝙩* 
wa.me/6281325714976
❏ *ɢʀᴜʙ ʙᴏᴛ ᴡᴀ:* 
https://chat.whatsapp.com/GEcTPvrxeOHL3HGg0YJD4Q
❏ *ᴄʜᴀɴɴᴇʟ ʏᴏᴜᴛᴜʙᴇ:* 
...
╚═════════════════❍
` 
                    akira.sendMessage(from, wew, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "*Subscribe jstfreenet*\n𝐓-𝐁𝐎𝐓 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('akiraganz/fake.jpg')} } }, caption: databaseuhy})
					break
				case 'donasi':
				case 'donate':
				if (!isRegistered) return reply(ind.noregis())
		...
		truteh = await getBuffer(`http://vignette2.wikia.nocookie.net/spongebob/images/3/33/Patrick_Star.svg/revision/latest?cb=20100724183918`)
		...
		tod = await getBuffer(`http://vignette2.wikia.nocookie.net/spongebob/images/3/33/Patrick_Star.svg/revision/latest?cb=20100724183918`)
		...
		if (isBanned) return reply(ind.baned())
			...
			if (!isGroup) return reply(ind.groupo())
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST 𝐓-𝐁𝐎𝐓 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋 」*\n\n${body.slice(4)}`)
						}
						reply('*_succes broadcast_* ')
					}
					break
            ...
            case 'creator':
                  ...
                  akira.sendMessage(from, 'Tuh kontak Isal <•_•> ,Jangan Lupa Save Ya (*>*)',MessageType.text, { quoted: mek} )
                  ini_buffer = await getBuffer("https://pm1.narvii.com/6482/f191b545a37f76db13b6bcde988820e5d3ae819c_hq.jpg")
		...
		akira.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('[𝐓-𝐁𝐎𝐓 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋]','yellow'), ('Command Tidak Ditemukan!!','red'), color(sender.split('@')[0]))
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
		}
